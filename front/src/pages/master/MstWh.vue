...
export default {
  ...
  async mounted() {
    ...
    this.columnLayout = [
      ...
      {
        dataField: "use_yn", headerText: "사용여부", width: 100, headerStyle: "aui-grid-required-header",
        renderer: {
          type: "DropDownListRenderer",
  ...
  methods: {
    ...
    removeRow() {
      const list = this.$refs.mstWhGrid.getCheckedRowItemsAll()
      if (list.length == 0) {
    ...
    saveRow() {
      ...
      const data = this.$gridGetCudData(this.$refs.mstWhGrid, ["plant_cd", "wh_cd", "use_yn"])
      if (data.length > 0) {
        this.loading = true
